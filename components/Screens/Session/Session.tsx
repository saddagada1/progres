...
const Session: React.FC<SessionProps> = ({ navigation, route }) => {
  const { institutionname, sessionid, sessionname, sessiongpa } = route.params;
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={[styles.container, { width: width * 0.9 }]}>
        ...
        <Text style={styles.gpa}>{sessiongpa ? sessiongpa : "TBD"}</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => item.semestersession === sessionid ? <SemesterCard semester={item} /> : null}
